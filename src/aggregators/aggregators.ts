        ...
        return function([num, denom]){
            return function(data, rowKey, colKey){
                return{
                    ...
                    sumNumDenom: 0,
                    push: function(record){
                        if(!isNaN(+record[num])){this.sumNum += +record[num]}
                        if(!isNaN(+record[denom])){this.sumNumDenom += +record[denom]}
                    },
                    value: function(){
        ...
        return function([num, denom]){
            return function(data, rowKey, colKey){
                return{
                    ...
                    push: function(record){
                        if(!isNaN(+record[num])){this.sumNum += +record[num]}
                        if(!isNaN(+record[denom])){this.sumDenom += +record[denom]}
                    },
                    value: function(){
                        let sign = upper ? 1 : -1
...
aggregatorTemplates.listUnique = function(s){
    return aggregatorTemplates.uniques(
        (function(x){return x.sort(naturalSort).join(s)}),
        (function(x){return x})
    )
}
