...
Element.prototype.pivot = function(input, inputOpts, locale="en"){
    ...
    let opts = deepMerge(true, localeDefaults, {...defaults, ...inputOpts})
    ...
    try {
        ...
        try {
            ...
            if(typeof console !== "undefined" && console !== null){console.error(error.stack)}
...
Element.prototype.pivotUI = function(input, inputOpts, overwrite=false, locale="en"){
    ...
    let defaults ={
        ...
        hiddenAttributes: [] as any[],
        hiddenFromAggregators: [] as any[],
        hiddenFromDragDrop: [] as any[],
        menuLimit: 500,
        cols: [] as any[],
        rows: [] as any[],
        vals: [] as any[],
    ...
    try {
        ...
        let uiTable: any = document.createElement("table")
        uiTable.className = "pvtUi"
        let rendererControl = "<td class=\"pvtUiCell\">"
        let renderer: any = "<select class=\"pvtRenderer\" id=\"renderer\">"
        let selOpts = ""
        ...
        let unusedAttrsVerticalAutoCutoff: number
        ...
        let altToggleClass = function(this: HTMLElement){
            ...
            (function(){
                ...
                if(values.length > opts.menuLimit){
                    ...
                    if(values.length > 5){
                        ...
                        let inputListener = function(this: HTMLInputElement){
                            let inputFilter = this.value.toLowerCase().trim()
                            let accept_gen = function(prefix: string, accepted: number[]){
                                return function(v){
                            ...
                            let accept: any
                            if(inputFilter.indexOf(">=") === 0){accept = accept_gen(">=",[1,0])}
                            else if(inputFilter.indexOf("<=") === 0){accept = accept_gen("<=",[-1,0])}
                            else if(inputFilter.indexOf(">") === 0){accept = accept_gen(">",[1])}
                            else if(inputFilter.indexOf("<") === 0){accept = accept_gen("<",[-1])}
                            else if(inputFilter.indexOf("~") === 0){accept =  function(v: string){
                                if(inputFilter.substring(1).trim().length === 0){
                                    return true
                                }
                                return v.toLowerCase().match(inputFilter.substring(1))
                            }}else{
                                accept = function(v: string){return v.toLowerCase().indexOf(inputFilter) !== -1}
                            }
                        ...
                        let selNoneLstnr = function(){
                            ...
                            if(selQuery.length !== 0){
                                for(let elem of selQuery){
                                    if(elem.parentElement?.parentElement!.style.display !== "none"){
                                        if((elem as HTMLInputElement).checked){
                ...
                let closeFilterBox = function(target){
                    ...
                    for(let el of cBoxes){
                        if((el as HTMLInputElement).checked){checked++}
                    }
                ...
                let cancelLstnr = function(this){
                    let cancelQuery = valueList.querySelectorAll('.changed')
                    for(let el of cancelQuery){
                        (el as HTMLInputElement).checked = (el as HTMLInputElement).checked ? false : true
                ...
                let triangleLinkLstnr = function(this){
                    let position = {
        ...
        let aggregator: any = "<select class=\"pvtAggregator\" id=\"aggr\">"
        ...
        let rowOrderLstnr = function(this) {
            this.dataset.order = ordering[this.dataset.order].next
            this.textContent = ordering[this.dataset.order].rowSymbol
            return refresh()
        }
        let rowOrderArrow: any = `<a role=\"button\" class=\"pvtRowOrder\" data-order=\"${opts.rowOrder}\" 
        id=\"rowOrder\">${ordering[opts.rowOrder].rowSymbol}</a>`

        let colOrderLstnr = function(this) {
            this.dataset.order = ordering[this.dataset.order].next
            this.textContent = ordering[this.dataset.order].colSymbol
            return refresh()
        }
        let colOrderArrow: any = `<a role=\"button\" class=\"pvtColOrder\" data-order=\"${opts.colOrder}\" 
        ...
        let pivotTable: any = "<td class=\"pvtRendererArea\" style=\"vertical-align:top;\" id=\"pivotTable\"></td></tr>"
        tr2 += pivotTable
        ...
        let colElemsFrag: any = document.createDocumentFragment()
        let rowElems = this.querySelector("#pvtRow")
        let rowElemsFrag: any = document.createDocumentFragment()
        ...
        if(opts.aggregatorName != null){
            ...
            (aggEl as HTMLInputElement).value = opts.aggregatorName
        }
        if(opts.rendererName != null){
            let renEl = this.querySelector("#renderer");
            (renEl as HTMLInputElement).value = opts.rendererName
        }
        ...
        let refreshDelayed = () => {
            ...
            for(let el of pvtRowList){
                subopts.rows.push((el as any).dataset.attrname)
            }
            let pvtColList = this.querySelectorAll('#pvtCol .pvtAttr')
            for(let el of pvtColList){
                subopts.cols.push((el as any).dataset.attrname)
            }
            ...
            for(let el of dDownList){
                if(numInputsToProcess === 0){
                    ...
                    if((el as HTMLInputElement).value !== ""){
                        vals.push((el as HTMLInputElement).value)
                    }
                }
            }
            ...
            if(numInputsToProcess !== 0){
                ...
                dDown?.addEventListener("change",function(){return refresh()})
            }
            ...
            if(initialRender){
                ...
                for(let el of elList){
                    (el as HTMLInputElement).value = vals[i]
                    i++
                }
                initialRender = false
            }
            ...
            for(let el of elList){
                ...
                if(!(el as HTMLInputElement).checked){
                    filter = (el as any).dataset.filter
                    filter = filter.split(',')
            ...
            let pivotUIOptions = {...opts, uiOpts}

            this.dataPivotUIOptions = pivotUIOptions
            //if requested make sure unused columns are in alphabetical order
            //To be implemented: Usage still undetermined
            if(opts.autoSortUnusedAttrs){alert("Found use of property: autoSortUnusedAttrs")}
            
            pivotTable.style.opacity = 1
            if(opts.onRefresh != null){opts.onRefresh(pivotUIOptions)}
